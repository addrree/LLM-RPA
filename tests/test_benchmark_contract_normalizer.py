...
def test_validator_allows_observe_page_helper_field_outside_business_contract():
    plan = _base_plan(
        ["items"],
        [
            {"step_id": 1, "action": "open_url", "args": {"url": "https://example.com"}},
            {"step_id": 2, "action": "observe_page", "args": {}, "save_as": "page_snapshot"},
            {
                ...
                "args": {"pattern": r"([A-Za-z]+)", "limit": 3, "fields": {"value": 1}},
                "save_as": "items",
            },
            {"step_id": 4, "action": "finish", "args": {}},
        ],
...
def test_navigation_contract_normalizer_does_not_raise_index_error_without_extract_step():
    plan = _base_plan(
        ["value"],
        [
            {"step_id": 1, "action": "open_url", "args": {"url": "https://example.com"}},
...
def test_validator_skips_contract_enforcement_for_negative_family_without_fixed_schema():
    plan = _base_plan(
        ["page_snapshot"],
        [
            {"step_id": 1, "action": "open_url", "args": {"url": "https://example.com"}},
    ...
    ctx = build_benchmark_context(
        ...
        required_top_level_fields=["status", "reason"],
    )
