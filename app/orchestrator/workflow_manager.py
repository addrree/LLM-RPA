...
def _canonicalize_family_steps(
    *,
    ...
    scenario_anchor_candidates: list[str] | None,
) -> list[dict]:
    if not steps:
        return steps
    ...
    for index, step in enumerate(steps):
        ...
        if action == "extract_value_near_anchor":
            has_anchor_text = _is_non_empty_str(args.get("anchor_text"))
            has_anchor_candidates = isinstance(args.get("anchor_candidates"), list) and any(
                _is_non_empty_str(item) for item in args.get("anchor_candidates", [])
            )
            if not has_anchor_text and not has_anchor_candidates:
                canonical_candidates = [
                    str(item).strip()
                    for item in (scenario_anchor_candidates or [])
                    if _is_non_empty_str(item)
                ]
                if not canonical_candidates:
                    canonical_candidates = ["Email", "Contact", "Support"]
                args["anchor_candidates"] = canonical_candidates
            ...
            if task_family == "anchored_value_extraction" and not _is_non_empty_str(step.get("save_as")):
                step["save_as"] = "value"
        ...
        if action == "extract_structured_items":
            default_limit = 5 if task_family == "multi_step_information_retrieval" else 10
            step["args"] = _canonical_structured_args(
                args,
        ...
        if task_family == "navigation_then_extraction" and action == "wait_for":
            ...
            if not has_wait_contract:
                ...
                if href_contains:
                    ...
                    args["selector"] = "h1"
        if action == "wait_for":
            has_wait_contract = any(_is_non_empty_str(args.get(key)) for key in ("selector", "url_contains", "text"))
            if not has_wait_contract:
                args["selector"] = "h1"
...
def normalize_benchmark_plan(
    ...
    normalized_steps = _canonicalize_family_steps(
        ...
        scenario_anchor_candidates=[
            str(item).strip()
            for item in (benchmark_context.get("scenario_anchor_candidates") or [])
            if str(item).strip()
        ],
    )
